                   ...
                   List constraints = R_NilValue) // instance-level constraints 
{
  ...
  if (!cl_hierarchy.containsElementNamed(cid.c_str())){
    List cl = cl_tree[cid];
    sc.push_back(std::stoi(cid)); // assume the leaf will be a salient cluster until proven otherwise
    ...
    if (!keep_children && cid != "0") {
      IntegerVector children = all_children(cl_hierarchy, std::stoi(cid)); // use all_children to prune subtrees
